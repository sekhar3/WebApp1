...
"""
    @author             : Somasekhar
    Description         : This method is used to link record for COVID PCR test(Issuer type: CDC)
"""


@pytest.mark.usefixtures("beforeClass", "beforeMethod")
class TestLinkRecordHealthCovidPcrDependent(unittest.TestCase):
    ...
    def test_link_record_health_covid_pcr_dependent(self):
        ...
        with allure.step("Selecting record type: covid pcr test"):
            self.upass_holder_portal.select_record_type_covid_pcr()
            LOGGER.info("Select record type: covid pcr test")
        ...
        with allure.step("Verifying covid pcr test health record"):
            self.upass_holder_portal.verify_covid_pcr_test()
            LOGGER.info("Verify covid pcr test health record")
